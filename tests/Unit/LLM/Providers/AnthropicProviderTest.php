...
class AnthropicProviderTest extends TestCase
{
    ...
    {
        $this->assertEquals('anthropic', $this->provider->getName());
    }
    ...
    public function test_generate_sends_correct_request(): void
    {
        ...
        $this->assertEquals(15, $response->promptTokens);
        $this->assertEquals(8, $response->completionTokens);
        $this->assertEquals('claude-3-5-sonnet-latest', $response->model);
    }

    public function test_chat_sends_messages_with_system_prompt_at_top_level(): void
    {
        $client = $this->createMockClient([
        ...
        // Check Anthropic-specific payload attributes
        $this->assertEquals('You are a coach', $body['system']);
        $this->assertCount(1, $body['messages']);
        $this->assertEquals('user', $body['messages'][0]['role']);
        $this->assertEquals('I am anxious', $body['messages'][0]['content']);
    ...
    public function test_handles_empty_choices(): void
    {
        ...
        $this->assertFalse($response->success);
        $this->assertStringContainsString('empty content', $response->error);
    }
    ...
    {
        $this->assertEquals('claude-3-5-sonnet-latest', $this->provider->getModel());
    }
